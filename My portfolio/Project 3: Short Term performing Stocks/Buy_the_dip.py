...
def versus_sp500():
    algo = backtesting()
    price = yf.download('^GSPC', start=(d + timedelta(days=252)), end=date.today(), parse_date=['Date'],
                        set_index=['Date'])
...
def mixed_vix():
    algo = backtesting()
    vix = yf.download('^VIX', start=(d + timedelta(days=252)), end=date.today(), parse_date=['Date'],
                      set_index=['Date'])
    vix = pd.DataFrame(vix['Close'])
    sp500 = yf.download('^GSPC ^VIX', start=(d + timedelta(days=260)), end=date.today(), parse_date=['Date'],
                        set_index=['Date'])
...
def Markowitz():
    ...
    prices = yf.download(str(lst), start=(date.today() - timedelta(days=300)), end=date.today(), parse_date='Date',
                         set_index='Date')
    close = (prices['Close'])
...
def mixed_vix_sp():
    algo = backtesting()
    vix = yf.download('^VIX', start=(d + timedelta(days=260)), end=date.today(), parse_date=['Date'],
                      set_index=['Date'])
    vix = pd.DataFrame(vix['Close'])
